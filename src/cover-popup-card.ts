      ...
      <div class="${fullscreen === true ? 'popup-wrapper':''}">
            <div class="popup-inner" @click="${e => this._close(e)}">
                ...
                <div class="range-holder" style="--slider-height: ${sliderHeight};--slider-width: ${sliderWidth};">
                    <input type="range" style="--slider-width: ${sliderWidth};--slider-height: ${sliderHeight}; --slider-border-radius: ${borderRadius}" .value="${stateObj.state === "off" ? 0 : Math.round(stateObj.attributes.current_position)}" @change=${e => this._setPosition(stateObj, e.target.value)}>
                </div>
