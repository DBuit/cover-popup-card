 ...
 * http://polymer.github.io/PATENTS.txt
 */
(window.litHtmlVersions||(window.litHtmlVersions=[])).push("1.1.2");const $=(t,...e)=>new f(t,e,"html",M)
/**
 * @license
 * Copyright (c) 2017 The Polymer Project Authors. All rights reserved.
 * This code may only be used under the BSD style license found at
      ...
      <div class="${!0===l?"popup-wrapper":""}">
        <div id="popup" class="popup-inner" @click="${t=>this._close(t)}">
          ...
          <div class="range-holder" style="--slider-height: ${h};--slider-width: ${d};">
              <input type="range" style="--slider-width: ${d};--slider-height: ${h}; --slider-border-radius: ${n};--slider-color:${p};--slider-thumb-color:${m};--slider-track-color:${g};" .value="${"off"===e.state?0:Math.round(e.attributes.current_position)}" @change=${t=>this._setPosition(e,t.target.value)}>
          </div>

          ${r&&r.length>0?$`
          <div class="action-holder">

              ${this.actionRows.map(t=>{u++;var e=0;return $`
                  <div class="action-row">
                  ${t.map(t=>(e++,$`
                      <div class="action" @click="${t=>this._activateAction(t)}" data-service="${u}#${e}">
                          <span class="color" style="background-color: ${t.color};border-color: ${t.color};">${t.icon?$`<ha-icon icon="${t.icon}" />`:$``}</span>
                          ${t.name?$`<span class="name">${t.name}</span>`:$``}
                      </div>
                    `))}
                  </div>
                `})}
          </div>`:$``}
          ${this.settings?$`<button class="bottom-right-btn" @click="${()=>this._openSettings()}">${this.config.settings.openButton?this.config.settings.openButton:"Settings"}</button>`:$``}
        </div>
        ${this.settings?$`
